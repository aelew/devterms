...
export function VoteActions({ definitionId, upvotes, downvotes }: VoteActionsProps) {
  const router = useRouter();
  const [optimisticUpvotes, updateOptimisticUpvotes] = useOptimistic(
    upvotes,
    (count, action: 'increment' | 'decrement') => count + (action === 'increment' ? 1 : -1)
  );
  ...
  const addDownvote = () => {
    ...
    localStorage.setItem('downvotes', JSON.stringify([...downvotes, definitionId]));
    setDownvoted(true);
  };

  const removeUpvote = () => {
    const upvotes = getUpvotes();
    localStorage.setItem('upvotes', JSON.stringify(upvotes.filter((id) => id !== definitionId)));
    setUpvoted(false);
  };

  const removeDownvote = () => {
    const downvotes = getDownvotes();
    localStorage.setItem('downvotes', JSON.stringify(downvotes.filter((id) => id !== definitionId)));
    setDownvoted(false);
  };
